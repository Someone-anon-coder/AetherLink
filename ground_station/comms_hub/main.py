    ...
    def datagram_received(self, data, addr):
        ...
        if self.data_type == 'telemetry':
            try:
                ...
                # Prepend telemetry header byte (0x01) and broadcast
                asyncio.create_task(broadcast(b'\x01' + json_message.encode('utf-8')))
            except DecodeError:
                print(f"Could not decode Telemetry from {addr}")

        elif self.data_type == 'video':
            # Prepend video header byte (0x02) and broadcast raw data
            asyncio.create_task(broadcast(b'\x02' + data))
...
async def main():
    ...
    # Start the UDP listener for Video on the GStreamer port
    video_transport, _ = await loop.create_datagram_endpoint(
        lambda: UdpProtocol(data_type='video'),
        local_addr=("0.0.0.0", 5600)  # New GStreamer port
    )
